...
} from "../interfaces/dao/deployables/IFreezeVotingGovernorV1.sol";
import {
...
} from "../interfaces/dao/deployables/IFreezeGuardGovernorV1.sol";
import {
 ...
 * 5. Deploy Governor module with strategy reference
 * 6. Complete circular initialization (Strategy ↔ Governor ↔ VotingConfigs)
    ...
    ERC165
{
    ...
    function setupSafe(
        ...
        GovernorGovernanceParams calldata governorGovernanceParams_,
    ...
    /**
     * @notice Deploys the complete Governor governance system
     * @dev Handles the complex deployment order and circular dependency resolution
     * between Governor, Strategy, and VotingConfigs.
     * @param salt_ Salt for deterministic deployment
     * @param governorGovernanceParams_ Complete governance configuration
     * @param newVotesERC20V1Addresses Addresses of newly deployed governance tokens
     * @return governorModuleAddress The deployed Governor module address (or zero if skipped)
     */
    function _deployGovernorGovernance(
        bytes32 salt_,
        GovernorGovernanceParams calldata governorGovernanceParams_,
        ...
        if (moduleGovernorV1Params.implementation != address(0)) {
            ...
            ISafe(address(this)).enableModule(governorModuleAddress);
        }

        return governorModuleAddress;
    }
     ...
     * Full initialization happens later with Governor and voting adapters.
     * @param salt_ Salt for deterministic deployment
    ...
    /**
     * @notice Deploys the Governor governance module
     ...
     * @param salt_ Salt for deterministic deployment
     * @param moduleGovernorV1Params Governor configuration parameters
     * @param strategyProxyAddress Address of the deployed strategy
     * @return governorProxyAddress The deployed Governor module address
     */
    function _deployModuleGovernor(
        bytes32 salt_,
     ...
     * The freeze guard is attached to either the Safe or Governor module.
     * @param salt_ Salt for deterministic deployment
     * @param freezeParams_ Freeze mechanism configurations
     * @param governorModuleAddress Address of Governor module (for freeze guard attachment)
     * @param newVotesERC20V1Addresses Addresses of newly deployed governance tokens
     */
    ...
    /**
     * @notice Deploys freeze voting contract (multisig, Governor-based, or standalone)
    ...
    function _deployFreezeVoting(
        ...
        if (freezeVotingGovernorV1Params.implementation != address(0)) {
            freezeVotingAddress = deployProxy(
                freezeVotingGovernorV1Params.implementation,
                abi.encodeCall(
                    IFreezeVotingGovernorV1.initialize,
                    (
                        freezeVotingGovernorV1Params.owner,
                        freezeVotingGovernorV1Params.freezeVotesThreshold,
                        freezeVotingGovernorV1Params.freezeProposalPeriod,
                        freezeVotingGovernorV1Params.parentGovernor,
                        freezeVotingGovernorV1Params.lightAccountFactory
                    )
                ),
                salt_
            );
     ...
     * @dev Can deploy multisig guard (on Safe) and/or Governor guard (on module).
     ...
     * @param governorModuleAddress Address of Governor module (for Governor guard)
     */
    ...
    /**
     * @notice Deploys Governor freeze guard and attaches to Governor module
     * @dev Guard blocks proposal execution when DAO is frozen.
     * Requires both freeze voting and Governor module to be deployed.
     * @param salt_ Salt for deterministic deployment
     * @param freezeGuardGovernorV1Params Governor guard configuration
     * @param freezeVotingAddress Address of freeze voting contract that controls freezing
     * @param governorModuleAddress Address of Governor module to attach guard to
     */
    function _deployFreezeGuardGovernor(
        bytes32 salt_,
        FreezeGuardGovernorV1Params memory freezeGuardGovernorV1Params,
        address freezeVotingAddress,
        address governorModuleAddress
    ) internal {
        if (freezeGuardGovernorV1Params.implementation != address(0)) {
            if (governorModuleAddress == address(0)) {
                revert GovernorModuleNotDeployed();
            }
            ...
            // add governor freeze guard to Governor module
            // Governor Module has same "setGuard" function signature as Safe
            ISafe(governorModuleAddress).setGuard(governorFreezeGuardAddress);
        }
    }
