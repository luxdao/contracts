...
export default buildModule('Deployables', m => {
  // Deploy Modules
  const moduleGovernorV1 = m.contract('ModuleGovernorV1');
  ...
  const freezeGuardGovernorV1 = m.contract('FreezeGuardGovernorV1');
  const freezeGuardMultisigV1 = m.contract('FreezeGuardMultisigV1');

  // Deploy Freeze Voting
  const freezeVotingGovernorV1 = m.contract('FreezeVotingGovernorV1');
