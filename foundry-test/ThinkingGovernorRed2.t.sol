...
contract ThinkingGovernorRed2Test is Test {
    ...
    /// WHICH group wins. Uses full withdrawal so the vote is actually dropped.
    ...
    /// @notice agreeing.length == bestCount exactly under churn (tally and collect use
    /// the SAME _bonded predicate in one settle, so they cannot disagree).
    ...
    /// @notice D1 (OK): treasury is ALSO an agreeing operator. fee + reward ADD (no stomp).
    ...
    /// @notice FIXED (E1): a full committee no longer settles pre-deadline.
    ...
    /// @notice MITIGATED (B1): the window floor is now 1 hour; a 60s window reverts.
    ...
    /// @notice A1 (OK): you can only submit a verdict YOU signed.
    ...
    /// @notice A2 (OK): per-operator digests differ; no single signature serves two ops.
    ...
    /// @notice F1 (OK): claimReward is nonReentrant + zeroes before the call (CEI).
