...
import {
  ...
  FreezeVotingGovernorV1__factory,
...
function createSetupSafeParams(overrides?: {
  ...
  governorGovernanceParams?: Partial<ISystemDeployerV1.GovernorGovernanceParamsStruct>;
  ...
  freezeGuardGovernorParams?: Partial<ISystemDeployerV1.FreezeGuardGovernorV1ParamsStruct>;
  freezeVotingMultisigParams?: Partial<ISystemDeployerV1.FreezeVotingMultisigV1ParamsStruct>;
  freezeVotingGovernorParams?: Partial<ISystemDeployerV1.FreezeVotingGovernorV1ParamsStruct>;
  ...
  // Default Governor governance params (all empty/zero)
  const governorGovernanceParams: ISystemDeployerV1.GovernorGovernanceParamsStruct = {
    ...
    ...overrides?.governorGovernanceParams,
  };
  ...
  const freezeParams: ISystemDeployerV1.FreezeParamsStruct = {
    freezeGuardParams: {
      ...
      freezeGuardGovernorV1Params: {
        implementation: ethers.ZeroAddress,
        owner: ethers.ZeroAddress,
        ...overrides?.freezeGuardGovernorParams,
      },
    },
    freezeVotingParams: {
      ...
      freezeVotingGovernorV1Params: {
        ...
        freezeVotesThreshold: 0,
        freezeProposalPeriod: 0,
        parentGovernor: ethers.ZeroAddress,
        lightAccountFactory: ethers.ZeroAddress,
        ...overrides?.freezeVotingGovernorParams,
      },
...
async function deploySafeWithSetup(params: {
  ...
  setupSafeParams: {
    ...
    governorGovernanceParams: ISystemDeployerV1.GovernorGovernanceParamsStruct;
  ...
  const setupSafeData = fixtureData.systemDeployer.interface.encodeFunctionData('setupSafe', [
    ...
    setupSafeParams.governorGovernanceParams,
...
// Helper function to find and verify ModuleGovernorV1 deployment and configuration
async function findAndVerifyModuleGovernorV1(params: {
  fixtureData: {
    systemDeployer: SystemDeployerV1;
    deployer: SignerWithAddress;
  };
  ...
  const governorAddress = await findProxyDeployed({
    fixtureData,
    receipt,
    implementation,
  });
  ...
  // Verify the module is enabled on the Safe
  const safeProxy = Safe__factory.connect(safeAddress, fixtureData.deployer);
  const isModuleEnabled = await safeProxy.isModuleEnabled(governorAddress);
  expect(isModuleEnabled).to.be.true;

  const governorProxy = ModuleGovernorV1__factory.connect(governorAddress, fixtureData.deployer);

  expect(await governorProxy.owner()).to.equal(owner);
  expect(await governorProxy.avatar()).to.equal(safeAddress);
  expect(await governorProxy.getFunction('target')()).to.equal(safeAddress);
  expect(await governorProxy.strategy()).to.equal(strategy);
  expect(await governorProxy.timelockPeriod()).to.equal(timelockPeriod);
  expect(await governorProxy.executionPeriod()).to.equal(executionPeriod);
...
// Helper function to find and verify FreezeGuardGovernor deployment and configuration
async function findAndVerifyFreezeGuardGovernorV1(params: {
  fixtureData: {
    systemDeployer: SystemDeployerV1;
    deployer: SignerWithAddress;
  };
  receipt: ContractTransactionReceipt;
  implementation: AddressLike;
  governorModuleAddress: string;
  owner: AddressLike;
  freezeVoting: string;
}) {
  const { fixtureData, receipt, implementation, governorModuleAddress, owner, freezeVoting } =
    params;

  const freezeGuardGovernorAddress = await findProxyDeployed({
    fixtureData,
    receipt,
    implementation,
  });

  // Verify the guard is set on the Governor module
  const governorModule = ModuleGovernorV1__factory.connect(
    governorModuleAddress,
    fixtureData.deployer,
  );
  const guard = await governorModule.getGuard();
  expect(guard).to.equal(freezeGuardGovernorAddress);

  // Connect to the deployed FreezeGuardGovernor proxy
  const freezeGuardGovernorProxy = FreezeGuardGovernorV1__factory.connect(
    freezeGuardGovernorAddress,
    fixtureData.deployer,
  );

  expect(await freezeGuardGovernorProxy.owner()).to.equal(owner);
  expect(await freezeGuardGovernorProxy.freezable()).to.equal(freezeVoting);

  return freezeGuardGovernorProxy;
}
...
// Helper function to find and verify FreezeVotingGovernor deployment and configuration
async function findAndVerifyFreezeVotingGovernorV1(params: {
  fixtureData: {
    systemDeployer: SystemDeployerV1;
    deployer: SignerWithAddress;
  };
  ...
  const freezeVotingGovernorAddress = await findProxyDeployed({
    fixtureData,
    receipt,
    implementation,
  });

  // Connect to the deployed FreezeVotingGovernor proxy
  const freezeVotingGovernorProxy = FreezeVotingGovernorV1__factory.connect(
    freezeVotingGovernorAddress,
    fixtureData.deployer,
  );

  expect(await freezeVotingGovernorProxy.owner()).to.equal(owner);
  expect(await freezeVotingGovernorProxy.freezeProposalPeriod()).to.equal(freezeProposalPeriod);
  expect(await freezeVotingGovernorProxy.freezeVotesThreshold()).to.equal(freezeVotesThreshold);
  expect(await freezeVotingGovernorProxy.parentGovernor()).to.equal(parentGovernor);
  expect(await freezeVotingGovernorProxy.lightAccountFactory()).to.equal(lightAccountFactory);

  return freezeVotingGovernorProxy;
}
...
// Helper function to find and verify the base Governor Governance setup
async function findAndVerifySafe(params: {
  fixtureData: {
    ...
    moduleGovernorV1: ModuleGovernorV1;
    votingWeightERC20V1: VotingWeightERC20V1;
    votingWeightERC721V1: VotingWeightERC721V1;
    voteTrackerERC20V1: VoteTrackerERC20V1;
    voteTrackerERC721V1: VoteTrackerERC721V1;
  ...
  moduleGovernorV1Params?: ISystemDeployerV1.ModuleGovernorV1ParamsStruct;
  votingConfigERC20V1Datas?: {
  ...
  freezeGuardMultisigV1Data?: {
    ...
    votingGovernorParams?: ISystemDeployerV1.FreezeVotingGovernorV1ParamsStruct;
  ...
  freezeGuardGovernorV1Data?: {
    guardParams: ISystemDeployerV1.FreezeGuardGovernorV1ParamsStruct;
    votingMultisigParams?: ISystemDeployerV1.FreezeVotingMultisigV1ParamsStruct;
    votingGovernorParams?: ISystemDeployerV1.FreezeVotingGovernorV1ParamsStruct;
  };
  numberOfNewContracts: number;
  ...
  let governorModule: ModuleGovernorV1 | undefined;
  if (moduleGovernorV1Params) {
    if (!strategyAddress) {
      throw new Error('Strategy is required to verify Governor module');
    }

    governorModule = await findAndVerifyModuleGovernorV1({
      fixtureData,
      receipt,
      safeAddress,
      owner: safeAddress,
  ...
  if (strategyV1Params) {
    if (!governorModule) {
      throw new Error('Governor module is required to verify a strategy');
    }
    ...
    await findAndVerifyStrategyV1({
      ...
      receipt,
      ...strategyV1Params,
      strategyAdmin: await governorModule.getAddress(),
      proposerAdapters: [
  ...
  if (freezeGuardMultisigV1Data) {
    const votingParamsCount = [
      freezeGuardMultisigV1Data.votingGovernorParams,
    ...
    if (freezeGuardMultisigV1Data.votingGovernorParams) {
      freezeVotingAddress = await (
        await findAndVerifyFreezeVotingGovernorV1({
          fixtureData,
          receipt,
          ...freezeGuardMultisigV1Data.votingGovernorParams,
        })
  ...
  if (freezeGuardGovernorV1Data) {
    if (!governorModule) {
      throw new Error('Governor module is required to verify freeze guard Governor');
    }

    const votingParamsCount = [
      freezeGuardGovernorV1Data.votingGovernorParams,
      freezeGuardGovernorV1Data.votingMultisigParams,
    ].filter(Boolean).length;
    ...
    if (freezeGuardGovernorV1Data.votingMultisigParams) {
      freezeVotingAddress = await (
        await findAndVerifyFreezeVotingMultisigV1({
          fixtureData,
          receipt,
    ...
    if (freezeGuardGovernorV1Data.votingGovernorParams) {
      freezeVotingAddress = await (
        await findAndVerifyFreezeVotingGovernorV1({
          fixtureData,
          receipt,
    ...
    await findAndVerifyFreezeGuardGovernorV1({
      fixtureData,
      receipt,
      governorModuleAddress: await governorModule.getAddress(),
      ...freezeGuardGovernorV1Data.guardParams,
      freezeVoting: freezeVotingAddress,
    });
  }
...
async function setupState() {
  ...
  const freezeGuardGovernorV1 = await new FreezeGuardGovernorV1__factory(deployer).deploy();
  const freezeVotingMultisigV1 = await new FreezeVotingMultisigV1__factory(deployer).deploy();
  const freezeVotingGovernorV1 = await new FreezeVotingGovernorV1__factory(deployer).deploy();
  const freezeVotingStandaloneV1 = await new FreezeVotingStandaloneV1__factory(deployer).deploy();
  const moduleGovernorV1 = await new ModuleGovernorV1__factory(deployer).deploy();
...
describe('SystemDeployerV1', () => {
  ...
  describe('setupSafe', () => {
    ...
    let freezeGuardGovernorParams: ISystemDeployerV1.FreezeGuardGovernorV1ParamsStruct;
    let freezeVotingMultisigParams: ISystemDeployerV1.FreezeVotingMultisigV1ParamsStruct;
    let freezeVotingGovernorParams: ISystemDeployerV1.FreezeVotingGovernorV1ParamsStruct;
    ...
    beforeEach(async () => {
      ...
      freezeGuardGovernorParams = {
        implementation: await fixtureData.freezeGuardGovernorV1.getAddress(),
        owner: fixtureData.user1.address,
      };
      ...
      freezeVotingGovernorParams = {
        implementation: await fixtureData.freezeVotingGovernorV1.getAddress(),
        owner: fixtureData.user1.address,
    ...
    describe('Deploy Multisig DAO', () => {
      ...
      describe('with Freeze Guards', () => {
        it('reverts with FreezeGuardGovernor', async () => {
          const setupSafeParams = createSetupSafeParams();
          ...
          // now update setupSafeParams to include a freezeGuardGovernorParams
          data.setupSafeParams.freezeParams.freezeGuardParams.freezeGuardGovernorV1Params =
            freezeGuardGovernorParams;
        ...
        it('succeeds with FreezeGuardMultisig and FreezeVotingGovernor contracts', async () => {
          const setupSafeParams = createSetupSafeParams({
            freezeVotingGovernorParams,
            freezeGuardMultisigParams,
          });
          ...
          await findAndVerifySafe({
            ...
            freezeGuardMultisigV1Data: {
              guardParams: freezeGuardMultisigParams,
              votingGovernorParams: freezeVotingGovernorParams,
            },
            numberOfNewContracts: 2,
          });
        });
    ...
    describe('Deploy Governor Governance', () => {
      let randomVotesERC20Contract: string;
      let randomVotesERC721Contract: string;
      let hatsContract: string;
      let moduleGovernorV1Params: ISystemDeployerV1.ModuleGovernorV1ParamsStruct;
      ...
      beforeEach(async () => {
        ...
        moduleGovernorV1Params = {
          implementation: await fixtureData.moduleGovernorV1.getAddress(),
          timelockPeriod: 3600,
      ...
      describe('Without Multisig', () => {
          ...
          describe('ERC20 voting adapters', () => {
            ...
            describe('One new VotesERC20 token', () => {
              ...
              it('reverts if the VotingConfigERC20 points to an invalid new token index', async () => {
                ...
                data.setupSafeParams.governorGovernanceParams.votingConfigParams.votingConfigERC20V1Params[0].newTokenIndex = 1;
          ...
          describe('ERC20 proposer adapters', () => {
            ...
            describe('One new VotesERC20 token', () => {
              ...
              it('reverts if the ProposerAdapterERC20 points to an invalid new token index', async () => {
                ...
                data.setupSafeParams.governorGovernanceParams.proposerAdapterParams.proposerAdapterERC20V1Params[0].newTokenIndex = 1;
        ...
        describe('with FreezeGuards', () => {
          describe('FreezeGuardMultisig configurations', () => {
            ...
            it('deploys with a FreezeGuardMultisig with FreezeVotingGovernor', async () => {
              const setupSafeParams = createSetupSafeParams({
                governorGovernanceParams: {
                  proposerAdapterParams: {
                    proposerAdapterERC20V1Params: [{ ...proposerAdapterERC20V1Params1 }],
            ...
            it('reverts if we deploy a FreezeGuardMultisig and NEITHER FreezeVoting contracts', async () => {
              ...
              // delete the freezeVotingGovernorParams by setting the implementation to zero address
              data.setupSafeParams.freezeParams.freezeVotingParams.freezeVotingGovernorV1Params.implementation =
                ethers.ZeroAddress;
          ...
          describe('FreezeGuardGovernor configurations', () => {
            it('deploys with a FreezeGuardGovernor with FreezeVotingMultisig', async () => {
              const setupSafeParams = createSetupSafeParams({
                governorGovernanceParams: {
                  proposerAdapterParams: {
                    proposerAdapterERC20V1Params: [{ ...proposerAdapterERC20V1Params1 }],
              ...
              await findAndVerifySafe({
                ...
                freezeGuardGovernorV1Data: {
                  guardParams: freezeGuardGovernorParams,
                  votingMultisigParams: freezeVotingMultisigParams,
                },
                numberOfNewContracts: 7,
              });
            ...
            it('deploys with a FreezeGuardGovernor with FreezeVotingGovernor', async () => {
              const setupSafeParams = createSetupSafeParams({
                governorGovernanceParams: {
                  proposerAdapterParams: {
                    proposerAdapterERC20V1Params: [{ ...proposerAdapterERC20V1Params1 }],
            ...
            it('reverts if we deploy a FreezeGuardGovernor and NEITHER FreezeVoting contract', async () => {
              const setupSafeParams = createSetupSafeParams({
                governorGovernanceParams: {
                  proposerAdapterParams: {
                    proposerAdapterERC20V1Params: [{ ...proposerAdapterERC20V1Params1 }],
              ...
              // now delete the freezeVotingGovernorParams by setting the implementation to zero address
              data.setupSafeParams.freezeParams.freezeVotingParams.freezeVotingGovernorV1Params.implementation =
                ethers.ZeroAddress;
            ...
            it('should revert when FreezeVotingStandalone is paired with FreezeGuardGovernor', async () => {
              // First, demonstrate that FreezeGuardGovernor works with FreezeVotingGovernor
              const setupSafeParams = createSetupSafeParams({
                governorGovernanceParams: {
                  proposerAdapterParams: {
                    proposerAdapterERC20V1Params: [{ ...proposerAdapterERC20V1Params1 }],
          ...
          describe('Both FreezeGuard contracts', () => {
            it('deploys with both FreezeGuards using FreezeVotingGovernor', async () => {
              const setupSafeParams = createSetupSafeParams({
                governorGovernanceParams: {
                  proposerAdapterParams: {
                    proposerAdapterERC20V1Params: [{ ...proposerAdapterERC20V1Params1 }],
