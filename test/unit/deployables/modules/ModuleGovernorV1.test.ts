...
// Helper functions for deploying GovernorV1 instances using ERC1967Proxy
async function deployGovernorProxy(
...
): Promise<ModuleGovernorV1> {
  // Combine selector and encoded params
  const fullInitData = ModuleGovernorV1__factory.createInterface().encodeFunctionData('initialize', [
    owner.address,
    avatar,
    target,
    strategyAddress,
  ...
  return ModuleGovernorV1__factory.connect(await proxy.getAddress(), owner);
}

// Helper function for deploying GovernorV1 using setUp instead of initialize
async function deployGovernorProxyWithSetUp(
  proxyDeployer: SignerWithAddress,
  implementation: string,
  owner: SignerWithAddress,
  avatar: string,
  ...
  const fullInitData = ModuleGovernorV1__factory.createInterface().encodeFunctionData('setUp', [
...
describe('ModuleGovernorV1', () => {
  ...
  let implementation: ModuleGovernorV1;
  ...
  beforeEach(async () => {
    ...
    implementation = await new ModuleGovernorV1__factory(proxyDeployer).deploy();
  ...
  describe('Initialization', () => {
    ...
    describe('Avatar and Target parameters', () => {
      it('should initialize with same avatar and target', async () => {
        ...
        expect(await governor.avatar()).to.equal(await avatar.getAddress());
        expect(await governor.getFunction('target')()).to.equal(await avatar.getAddress());
      });
      ...
      it('should initialize with different target than avatar', async () => {
        ...
        expect(await governor.avatar()).to.equal(await avatar.getAddress());
        expect(await governor.getFunction('target')()).to.equal(user.address);
      });
      ...
      it('should allow zero address avatar', async () => {
        ...
        expect(await governor.avatar()).to.equal(ethers.ZeroAddress);
      });
      ...
      it('should allow zero address target', async () => {
        ...
        expect(await governor.getFunction('target')()).to.equal(ethers.ZeroAddress);
      });
    ...
    describe('Strategy parameter', () => {
      it('should initialize with a valid strategy', async () => {
        ...
        expect(await governor.strategy()).to.equal(localMockStrategyAddress);
      });
    });
    ...
    describe('Timelock and Execution periods', () => {
      it('should initialize with non-zero periods', async () => {
        ...
        expect(await governor.timelockPeriod()).to.equal(timelockPeriod);
        expect(await governor.executionPeriod()).to.equal(executionPeriod);
      });
      ...
      it('should initialize with zero periods', async () => {
        ...
        expect(await governor.timelockPeriod()).to.equal(0);
        expect(await governor.executionPeriod()).to.equal(0);
      });
      ...
      it('should initialize with max uint32 periods', async () => {
        ...
        expect(await governor.timelockPeriod()).to.equal(maxUint32);
        expect(await governor.executionPeriod()).to.equal(maxUint32);
      });
    });
    ...
    describe('Reinitialization prevention', () => {
      it('should not allow reinitialization', async () => {
        ...
        ).to.be.revertedWithCustomError(governor, 'InvalidInitialization');
      });

      it('Should have initialization disabled in the implementation', async function () {
        const implementationContract = ModuleGovernorV1__factory.connect(masterCopy, proxyDeployer);
    ...
    describe('setUp function', () => {
      it('should correctly initialize contract when using setUp', async () => {
        governor = await deployGovernorProxyWithSetUp(
          proxyDeployer,
          masterCopy,
          owner,
          await avatar.getAddress(),
        ...
        expect(await governor.avatar()).to.equal(await avatar.getAddress());
        expect(await governor.getFunction('target')()).to.equal(await avatar.getAddress());
        expect(await governor.strategy()).to.equal(mockStrategyAddress);
        expect(await governor.timelockPeriod()).to.equal(100);
        expect(await governor.executionPeriod()).to.equal(200);
      });
      ...
      it('should not allow setUp to be called again after initialization', async () => {
        ...
        await expect(governor.setUp(innerParams)).to.be.revertedWithCustomError(
          governor,
  ...
  describe('Proposal Tests', () => {
    ...
    beforeEach(async () => {
      ...
      // Deploy Governor with initial strategy
      governor = await deployGovernorProxy(
        proxyDeployer,
        masterCopy,
        owner,
    ...
    describe('Proposal Management', () => {
      ...
      it('should allow proposer to submit proposal', async () => {
        ...
        const eventFragment = governor.interface.getEvent('ProposalCreated');
        ...
        const event = governor.interface.decodeEventLog(
      ...
      it('should not allow non-proposer to submit proposal', async () => {
        ...
        ).to.be.revertedWithCustomError(governor, 'InvalidProposer');
      });
      ...
      it('should allow submitting a proposal with zero transactions and verify its state progression', async () => {
        ...
        const proposalId = await governor.totalProposalCount();
        ...
        expect(await governor.totalProposalCount()).to.equal(proposalId + 1n);
        const [, txHashes, , ,] = await governor.getProposal(Number(proposalId));
        ...
        const currentState = await governor.proposalState(Number(proposalId));
    ...
    describe('Proposal Transaction Management', () => {
      it('should revert when accessing proposalState for an uninitialized proposal', async () => {
        await expect(governor.proposalState(999)).to.be.revertedWithCustomError(
          governor,
          'InvalidProposal',
        );
      });
      ...
      it('should revert when accessing proposalState with proposalId equal to totalProposalCount', async () => {
        ...
        await expect(governor.proposalState(0)).to.be.revertedWithCustomError(
          governor,
          'InvalidProposal',
        );
        ...
        await expect(governor.proposalState(1)).to.be.revertedWithCustomError(
          governor,
          'InvalidProposal',
        );
      });
      ...
      it('should return correct hashes for multiple transactions', async () => {
        ...
        // Get hashes directly
        const hash1 = await governor.getTxHash(tx1);
        const hash2 = await governor.getTxHash(tx2);

        // Verify proposal tx hashes match
        expect(await governor.getProposalTxHash(0, 0)).to.equal(hash1);
        expect(await governor.getProposalTxHash(0, 1)).to.equal(hash2);

        // Also verify getProposalTxHashes returns all hashes
        const hashes = await governor.getProposalTxHashes(0);
    ...
    describe('getProposal', () => {
      it('should return correct details for an existing proposal', async () => {
        ...
        const expectedTxHash = await governor.getTxHash(proposalTx);

        const [strategy, txHashes, timelock, execution, counter] =
          await governor.getProposal(proposalId);
      ...
      it('should return default/zero values for a non-existent proposalId (higher than totalProposalCount)', async () => {
        const nonExistentProposalId = (await governor.totalProposalCount()) + 1n; // Get current count and add 1
        ...
        const [strategy, txHashes, timelock, execution, counter] = await governor.getProposal(
      ...
      it('should return details correctly after partial execution', async () => {
        ...
        await avatar.enableModule(await governor.getAddress());
        ...
        await governor.executeProposal(proposalId, [tx1]);

        const [, , , , counter] = await governor.getProposal(proposalId);
    ...
    describe('Proposal State and Execution', () => {
      ...
      it('should track proposal state correctly', async () => {
        ...
        expect(await governor.proposalState(proposalId)).to.equal(0); // ACTIVE
        ...
        expect(await governor.proposalState(proposalId)).to.equal(1); // TIMELOCKED

        // Move past timelock
        await time.increase(TIMELOCK_PERIOD);

        // Should be executable
        expect(await governor.proposalState(proposalId)).to.equal(2); // EXECUTABLE

        // Move past execution period
        await time.increase(EXECUTION_PERIOD);

        // Should be expired
        expect(await governor.proposalState(proposalId)).to.equal(4); // EXPIRED
      });
      ...
      it('should execute proposal transactions when executable', async () => {
        ...
        // Execute proposal
        await governor.executeProposal(proposalId, [proposalTx]);
      ...
      it('should not execute proposal before timelock period', async () => {
        ...
        await expect(
          governor.executeProposal(proposalId, [proposalTx]),
        ).to.be.revertedWithCustomError(governor, 'ProposalNotExecutable');
      });
      ...
      describe('Timestamp-based proposal state transitions', () => {
        ...
        it('should handle exact boundary conditions in timestamp transitions', async () => {
          ...
          expect(await governor.proposalState(proposalId)).to.equal(0); // Should still be ACTIVE at exactly the end timestamp

          // One second after voting end
          await time.increaseTo(currentTimestamp + 101);
          expect(await governor.proposalState(proposalId)).to.equal(1); // Should be TIMELOCKED after end timestamp

          // At exactly the end of timelock period
          await time.increaseTo(currentTimestamp + 101 + TIMELOCK_PERIOD);
          expect(await governor.proposalState(proposalId)).to.equal(2); // Should be EXECUTABLE

          // At exactly the end of execution period
          await time.increaseTo(currentTimestamp + 101 + TIMELOCK_PERIOD + EXECUTION_PERIOD);
          expect(await governor.proposalState(proposalId)).to.equal(4); // Should be EXPIRED at exactly end of execution period
        });
      });
      ...
      it('should correctly transition to FAILED state if strategy returns isPassed as false', async () => {
        ...
        expect(await governor.proposalState(proposalIdToFail)).to.equal(5); // FAILED (Enum.ProposalState.FAILED)
      });
    });
    ...
    describe('Proposal Execution Edge Cases', () => {
      ...
      describe('Partial execution', () => {
        ...
        it('should allow partial execution of proposal transactions', async () => {
          // Execute only the first transaction
          await governor.executeProposal(0, [tx1]);

          // Verify first transaction was executed
          expect(await mockToken.balanceOf(user.address)).to.equal(100);

          // Verify execution counter was incremented
          const [, , , , executionCounter] = await governor.getProposal(0);
        ...
        it('should allow execution of remaining transactions after partial execution', async () => {
          ...
          await governor.executeProposal(0, [tx2]);
          ...
          const [, , , , finalExecutionCounter] = await governor.getProposal(0);
          expect(finalExecutionCounter).to.equal(2);

          // Verify proposal state is now EXECUTED
          expect(await governor.proposalState(0)).to.equal(3); // EXECUTED
        });
      });
      ...
      it('should revert on execution counter overflow', async () => {
        ...
        expect(await governor.proposalState(1)).to.equal(2); // EXECUTABLE

        // First execute all transactions
        await governor.executeProposal(1, [tx1, tx2]);

        // Try to execute more transactions than exist
        await expect(governor.executeProposal(1, [tx1])).to.be.revertedWithCustomError(
          governor,
          'InvalidTxs',
        );
      });
      ...
      describe('executeProposal Specific Reverts', () => {
        ...
        beforeEach(async () => {
          ...
          proposalId = Number(await governor.totalProposalCount());
          await governor
            .connect(proposer)
        ...
        it('should revert with InvalidTxs if targets array is empty', async () => {
          await expect(governor.executeProposal(proposalId, [])).to.be.revertedWithCustomError(
            governor,
            'InvalidTxs',
          );
        });
        ...
        it('should revert with InvalidTxHash if provided tx details do not match stored hash', async () => {
          ...
          await expect(
            governor.executeProposal(proposalId, [invalidTx]),
          ).to.be.revertedWithCustomError(governor, 'InvalidTxHash');
        });
      });
    });
    ...
    describe('Transaction Hash Generation', () => {
      it('should generate correct transaction hash', async () => {
        ...
        const txHash = await governor.getTxHash(tx);
      ...
      it('should generate different hashes for different transactions', async () => {
        ...
        const txHash1 = await governor.getTxHash(tx1);
        const txHash2 = await governor.getTxHash(tx2);
    ...
    describe('generateTxHashData', () => {
      ...
      beforeEach(async () => {
        // Re-deploy Governor and mocks if they are not available in this scope
        // For this specific case, Governor is deployed in the parent 'Proposal Tests' scope
      ...
      it('should generate a non-empty bytes string', async () => {
        const hashData = await governor.generateTxHashData(txParams, nonce);
      ...
      it('should produce different hash data for different nonces', async () => {
        const hashData1 = await governor.generateTxHashData(
          txParams,
          0n, // Nonce 0
        );
        const hashData2 = await governor.generateTxHashData(
          txParams,
          1n, // Nonce 1
        );
      ...
      it('should produce different hash data for different to addresses', async () => {
        const hashData1 = await governor.generateTxHashData(txParams, nonce);

        txParams.to = owner.address; // Different 'to'
        const hashData2 = await governor.generateTxHashData(txParams, nonce);
        expect(hashData1).to.not.equal(hashData2);
      });
  ...
  describe('Owner Functions', () => {
    ...
    describe('updateTimelockPeriod', () => {
      ...
      it('should allow owner to update timelock period', async () => {
        await expect(governor.connect(owner).updateTimelockPeriod(NEW_TIMELOCK_PERIOD))
          .to.emit(governor, 'TimelockPeriodUpdated')
          .withArgs(NEW_TIMELOCK_PERIOD);
        expect(await governor.timelockPeriod()).to.equal(NEW_TIMELOCK_PERIOD);
      });

      it('should not allow non-owner to update timelock period', async () => {
        await expect(governor.connect(nonOwner).updateTimelockPeriod(NEW_TIMELOCK_PERIOD))
      ...
      it('should apply updated timelock period to new proposals', async () => {
        ...
        await governor.connect(owner).updateTimelockPeriod(NEW_TIMELOCK_PERIOD);
        ...
        const [, , timelock, ,] = await governor.getProposal(proposalId);
      ...
      it('should not apply updated timelock period to existing proposals', async () => {
        ...
        const [, , timelock, ,] = await governor.getProposal(existingProposalId);
        ...
        const [, , newTimelock, ,] = await governor.getProposal(newProposalId);
    ...
    describe('updateExecutionPeriod', () => {
      ...
      it('should allow owner to update execution period', async () => {
        await expect(governor.connect(owner).updateExecutionPeriod(NEW_EXECUTION_PERIOD))
          .to.emit(governor, 'ExecutionPeriodUpdated')
          .withArgs(NEW_EXECUTION_PERIOD);
        expect(await governor.executionPeriod()).to.equal(NEW_EXECUTION_PERIOD);
      });

      it('should not allow non-owner to update execution period', async () => {
        await expect(governor.connect(nonOwner).updateExecutionPeriod(NEW_EXECUTION_PERIOD))
          .to.be.revertedWithCustomError(governor, 'OwnableUnauthorizedAccount')
          .withArgs(nonOwner.address);
      });

      it('should apply updated execution period to new proposals', async () => {
        await governor.connect(owner).updateExecutionPeriod(NEW_EXECUTION_PERIOD);
        ...
        const [, , , executionPeriod] = await governor.getProposal(proposalId);
      ...
      it('should not apply updated execution period to existing proposals', async () => {
        ...
        const [, , , executionPeriod] = await governor.getProposal(existingProposalId);
        ...
        const newProposalId = 1;
        const [, , , newExecution] = await governor.getProposal(newProposalId);
    ...
    describe('updateStrategy', () => {
      ...
      it('should allow owner to update strategy', async () => {
        await expect(governor.connect(owner).updateStrategy(newMockStrategyAddress))
          .to.emit(governor, 'StrategyUpdated')
          .withArgs(newMockStrategyAddress);
        expect(await governor.strategy()).to.equal(newMockStrategyAddress);
      });

      it('should not allow non-owner to update strategy', async () => {
        await expect(governor.connect(nonOwner).updateStrategy(newMockStrategyAddress))
          .to.be.revertedWithCustomError(governor, 'OwnableUnauthorizedAccount')
          .withArgs(nonOwner.address);
      });
      ...
      it('should not allow updating to a zero address', async () => {
        await expect(
          governor.connect(owner).updateStrategy(ethers.ZeroAddress),
        ).to.be.revertedWithCustomError(governor, 'InvalidStrategy');
      });

      it('should apply updated strategy to new proposals', async () => {
        await governor.connect(owner).updateStrategy(newMockStrategyAddress);
        ...
        const [strategyAddress, , , ,] = await governor.getProposal(proposalId);
      ...
      it('should not apply updated strategy to existing proposals', async () => {
        ...
        const [strategyAddress, , , ,] = await governor.getProposal(existingProposalId);
        ...
        const newProposalId = 1;
        const [newStrategy, , , ,] = await governor.getProposal(newProposalId);
      ...
      it("should use the proposal's original strategy for proposalState determination", async () => {
        ...
        // The governor instance is already configured with mockStrategyAddress in the parent beforeEach
        await governor
          .connect(proposer)
          .submitProposal(
            [proposalTx],
        ...
        expect(await governor.proposalState(existingProposalId)).to.equal(1);
        ...
        expect(await governor.proposalState(existingProposalId)).to.equal(2);
      });
    });
  });
  ...
  describe('ERC165 supportsInterface', function () {
    let governorInstance: ModuleGovernorV1;

    beforeEach(async function () {
      // Deploy a new instance for testing
      governorInstance = await deployGovernorProxy(
        proxyDeployer,
        masterCopy,
        proxyDeployer,
        ethers.ZeroAddress,
  ...
  describe('UUPS Upgradeability', function () {
    ...
    runUUPSUpgradeabilityTests({
      getContract: () => governor as unknown as UUPSUpgradeable,
      createNewImplementation: async () => {
        const newImplementation = await new ModuleGovernorV1__factory(owner).deploy();
  ...
  describe('Deployment Block', () => {
    ...
    runDeploymentBlockTests({
      getContract: () => governor as unknown as IDeploymentBlock,
    });
  });
  ...
  describe('InitializerEventEmitter', () => {
    runInitializerEventEmitterTests({
      contractFactory: ModuleGovernorV1__factory as unknown as ContractFactory,
