...
async function deployGovernorFreezeVotingProxy(
  proxyDeployer: SignerWithAddress,
  implementationAddress: string,
  owner: string,
  freezeVotesThreshold: bigint,
  freezeProposalPeriod: number,
  parentGovernorAddress: string,
  lightAccountFactoryAddress: string,
): Promise<FreezeVotingGovernorV1> {
  const initData = FreezeVotingGovernorV1__factory.createInterface().encodeFunctionData(
    'initialize',
    [
      owner,
      freezeVotesThreshold,
      freezeProposalPeriod,
      parentGovernorAddress,
  ...
  return FreezeVotingGovernorV1__factory.connect(await proxy.getAddress(), proxyDeployer);
}

describe('FreezeVotingGovernorV1', () => {
  let deployer: SignerWithAddress;
  let owner: SignerWithAddress;
  let voter1: SignerWithAddress;
  let voter2: SignerWithAddress;

  let governorFreezeVoting: FreezeVotingGovernorV1;
  let mockParentGovernor: MockModuleGovernorV1;
  ...
  let governorFreezeVotingImplementationAddress: string;
  ...
  async function fixture() {
    ...
    const implFactory = new FreezeVotingGovernorV1__factory(d);
    const impl = await implFactory.deploy();
    await impl.waitForDeployment();
    const implAddr = await impl.getAddress();

    const mockGovernorFactory = new MockModuleGovernorV1__factory(d);
    const mGovernor = await mockGovernorFactory.deploy();
    await mGovernor.waitForDeployment();
    ...
    await mGovernor.connect(d).updateStrategy(await mStrategy.getAddress()); // Configure mock Governor with mock Strategy
    ...
    return {
      ...
      voter1: v1,
      voter2: v2,
      governorFreezeVotingImplementationAddress: implAddr,
      mockParentGovernor: mGovernor,
  ...
  beforeEach(async () => {
    ...
    voter1 = f.voter1;
    voter2 = f.voter2;
    governorFreezeVotingImplementationAddress = f.governorFreezeVotingImplementationAddress;
    mockParentGovernor = f.mockParentGovernor;
    mockStrategy = f.mockStrategy;
    votingConfig1 = f.votingConfig1;
    mockLightAccountFactory = f.mockLightAccountFactory;

    governorFreezeVoting = await deployGovernorFreezeVotingProxy(
      deployer,
      governorFreezeVotingImplementationAddress,
      owner.address,
      DEFAULT_FREEZE_VOTES_THRESHOLD,
      DEFAULT_FREEZE_PROPOSAL_PERIOD,
      await mockParentGovernor.getAddress(),
  ...
  describe('Initialization', () => {
    it('should initialize with correct parameters', async () => {
      expect(await governorFreezeVoting.owner()).to.equal(owner.address);
      expect(await governorFreezeVoting.freezeVotesThreshold()).to.equal(
        DEFAULT_FREEZE_VOTES_THRESHOLD,
      );
      expect(await governorFreezeVoting.freezeProposalPeriod()).to.equal(
        DEFAULT_FREEZE_PROPOSAL_PERIOD,
      );
      expect(await governorFreezeVoting.parentGovernor()).to.equal(
        await mockParentGovernor.getAddress(),
      );
      expect(await governorFreezeVoting.lightAccountFactory()).to.equal(
        mockLightAccountFactory.target as string,
      );
    });
    ...
    it('should not allow reinitialization', async () => {
      await expect(
        governorFreezeVoting.initialize(
          owner.address,
          DEFAULT_FREEZE_VOTES_THRESHOLD,
          DEFAULT_FREEZE_PROPOSAL_PERIOD,
          await mockParentGovernor.getAddress(),
      ...
      ).to.be.revertedWithCustomError(governorFreezeVoting, 'InvalidInitialization');
    });

    it('implementation contract should have initializers disabled', async () => {
      const newImpl = FreezeVotingGovernorV1__factory.connect(
        governorFreezeVotingImplementationAddress,
        deployer,
      );
      await expect(
  ...
  describe('parentGovernor()', () => {
    it('should return the correct parent Governor contract address', async () => {
      expect(await governorFreezeVoting.parentGovernor()).to.equal(
        await mockParentGovernor.getAddress(),
      );
    });
  ...
  describe('castFreezeVote', () => {
    ...
    beforeEach(async () => {
      ...
      const freezeVotingAddress = await governorFreezeVoting.getAddress();
    ...
    it('should initiate a new freeze proposal period if none active and record a vote', async () => {
      const initialFreezeProposalCreated = await governorFreezeVoting.freezeProposalCreated();
      ...
      const tx = await governorFreezeVoting.connect(voter1).castFreezeVote(votingConfigData, 0n);
      const receipt = await tx.wait();
      const block = await ethers.provider.getBlock(receipt!.blockNumber!);
      const txTimestamp = BigInt(block!.timestamp);

      await expect(tx)
        .to.emit(governorFreezeVoting, 'FreezeProposalCreated')
        .withArgs(voter1.address, await mockStrategy.getAddress());

      await expect(tx)
        .to.emit(governorFreezeVoting, 'FreezeVoteCast')
        .withArgs(voter1.address, voteAmount);

      expect(await governorFreezeVoting.freezeProposalCreated()).to.equal(txTimestamp - 1n);
      expect(await governorFreezeVoting.freezeProposalVoteCount()).to.equal(voteAmount);
      expect(await governorFreezeVoting.freezeProposalStrategy()).to.equal(
    ...
    it('should use an existing active freeze proposal period', async () => {
      ...
      await governorFreezeVoting.connect(voter1).castFreezeVote(votingConfigData, 0n);
      const firstProposalCreatedTimestamp = await governorFreezeVoting.freezeProposalCreated();
      const firstVoteCount = await governorFreezeVoting.freezeProposalVoteCount();
      ...
      const txPromise = governorFreezeVoting.connect(voter2).castFreezeVote(votingConfigData, 0n); // voter2 casts a vote

      // Should NOT emit FreezeProposalCreated again
      await expect(txPromise).to.not.emit(governorFreezeVoting, 'FreezeProposalCreated');
      await expect(txPromise)
        .to.emit(governorFreezeVoting, 'FreezeVoteCast')
        .withArgs(voter2.address, newVoteAmount);

      expect(await governorFreezeVoting.freezeProposalCreated()).to.equal(
        firstProposalCreatedTimestamp,
      );
      expect(await governorFreezeVoting.freezeProposalVoteCount()).to.equal(
    ...
    it('should start a new proposal period if current one has expired', async () => {
      ...
      const tx = await governorFreezeVoting.connect(voter2).castFreezeVote(votingConfigData, 0n);
      const receipt = await tx.wait();
      const block = await ethers.provider.getBlock(receipt!.blockNumber!);
      ...
      const newProposalCreatedTimestamp = await governorFreezeVoting.freezeProposalCreated();
      ...
      expect(await governorFreezeVoting.freezeProposalVoteCount()).to.equal(newVoteAmount);
    });
    ...
    it('should revert when trying to vote with an already used config', async () => {
      ...
      const proposalId = await governorFreezeVoting.freezeProposalCreated();
      await expect(governorFreezeVoting.connect(voter1).castFreezeVote(votingConfigData, 0n))
    ...
    it('should allow voting with different configs even if one was already used', async () => {
      ...
      await expect(governorFreezeVoting.connect(voter1).castFreezeVote(votingConfigData2, 0n))
        .to.emit(governorFreezeVoting, 'FreezeVoteCast')
        .withArgs(voter1.address, voteAmount);

      expect(await governorFreezeVoting.freezeProposalVoteCount()).to.equal(voteAmount * 2n);
    });
    ...
    it('should revert when voter has zero weight for a config', async () => {
      ...
      await expect(governorFreezeVoting.connect(voter1).castFreezeVote(votingConfigData, 0n))
        .to.be.revertedWithCustomError(governorFreezeVoting, 'NoVotingWeight')
    ...
    it('should emit FreezeVoteRecorded events for each config', async () => {
      ...
      const tx = await governorFreezeVoting.connect(voter1).castFreezeVote(multiConfigVoteData, 0n);

      // Check for both FreezeVoteRecorded events
      await expect(tx)
        .to.emit(governorFreezeVoting, 'FreezeVoteRecorded')
        .withArgs(
          voter1.address,
          await governorFreezeVoting.freezeProposalCreated(),
          voteAmount,
          '0x',
        );
    ...
    it('should revert with correct parameters when voting with already used config at different index', async () => {
      ...
      await governorFreezeVoting.connect(voter1).castFreezeVote(votingConfigData2, 0n);
    ...
    it('should revert with correct parameters including non-empty vote data', async () => {
      ...
      // First vote should succeed
      await governorFreezeVoting.connect(voter1).castFreezeVote(votingConfigDataERC721, 0n);

      // Second vote should revert with the encoded token IDs
      const proposalId = await governorFreezeVoting.freezeProposalCreated();
      await expect(governorFreezeVoting.connect(voter1).castFreezeVote(votingConfigDataERC721, 0n))
    ...
    it('should revert with correct config index when error occurs in multi-config voting', async () => {
      ...
      await expect(governorFreezeVoting.connect(voter1).castFreezeVote(multiConfigVoteData, 0n))
        .to.be.revertedWithCustomError(governorFreezeVoting, 'NoVotingWeight')
  ...
  describe('Version', () => {
    it('should return the correct version', async () => {
      expect(await governorFreezeVoting.version()).to.equal(1);
    });
  });

  describe('ERC165 supportsInterface', () => {
    runSupportsInterfaceTests({
      getContract: () => governorFreezeVoting,
  ...
  describe('InitializerEventEmitter', () => {
    runInitializerEventEmitterTests({
      contractFactory: FreezeVotingGovernorV1__factory,
      masterCopy: () => governorFreezeVotingImplementationAddress,
      deployer: () => deployer,
