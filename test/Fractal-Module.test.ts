  ...
  describe('Fractal Module', () => {
    it('Supports the expected ERC165 interface', async () => {
      ...
      await expect(multiSendCallOnly.multiSend(safeTx))
        ...
        .withArgs(await gnosisSafe.getAddress(), await gnosisSafeL2Singleton.getAddress());
    });
    ...
    it('Authorized users may exec TXs', async () => {
      ...
      await expect(fractalModule.connect(owner1).execTx(txData)).to.be.revertedWithCustomError(
        fractalModule,
        'TxFailed',
      );
