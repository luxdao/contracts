...
Lux DAO Contracts is a Hardhat-based smart contract project implementing the **Lux Governor Protocol** - a Safe Zodiac Module framework for composable governance in DAOs. The project enables modular voting strategies with support for multiple token standards (ERC20, ERC721, Hats Protocol) integrated with Gnosis Safe.

## Lux Governor Protocol

A Safe module which allows for composable governance.

The Governor Protocol conforms to the [Zodiac](https://github.com/gnosis/zodiac) pattern for Safe modules.

The **ModuleGovernorV1** contract acts as a central manager of DAO Proposals, maintaining the specifications of the transactions that comprise a Proposal.

All voting details are delegated to `StrategyV1` implementations, of which a DAO can have any number.
...
1. **ModuleGovernorV1** (`contracts/deployables/modules/ModuleGovernorV1.sol`)
...
2. **ModuleFractalV1** (`contracts/deployables/modules/ModuleFractalV1.sol`)

   - Enables parent-child DAO relationships (SubDAO support)
...
3. **StrategyV1** (`contracts/deployables/strategies/StrategyV1.sol`)
...
4. **Voting Adapters** (`contracts/deployables/strategies/voting-weight/`)
...
5. **Proposer Adapters** (`contracts/deployables/strategies/proposer-adapters/`)
...
1. Submit proposal through ModuleGovernorV1 with transactions and metadata
...
- **Safe Integration**: ModuleGovernorV1 acts as a Safe module, executing transactions through `execTransactionFromModule()`
...
- **Examples**:
  - ModuleGovernorV1: Central governance module
  - StrategyV1: Voting strategy implementation
  - VotingWeight adapters: Token-based voting mechanisms
...
```shell
git clone https://github.com/luxfi/dao.git
cd dao/contracts
```
...
```shell
npx hardhat test test/unit/deployables/modules/ModuleGovernorV1.test.ts
```
...
For example, to deploy to Lux Mainnet:

```shell
npx hardhat ignition deploy ignition/modules/DeployAll.ts --network lux --strategy create2 --verify
```

This command will:

- Deploy all contracts using CREATE2 for deterministic addresses
- Automatically verify contracts on the block explorer
...
```shell
npx hardhat ignition deploy ignition/modules/deployables/DeployablesModule.ts --network <network> --strategy create2 --verify
npx hardhat ignition deploy ignition/modules/services/ServicesModule.ts --network <network> --strategy create2 --verify
npx hardhat ignition deploy ignition/modules/singletons/SingletonsModule.ts --network <network> --strategy create2 --verify
npx hardhat ignition deploy ignition/modules/utilities/UtilitiesModule.ts --network <network> --strategy create2 --verify
```
...
### Usage with @luxfi/standard

This package re-exports ABIs from `@luxfi/standard`. You can import governance ABIs directly:

```typescript
import { GovernorAbi, StrategyAbi, SubDAOAbi } from '@luxfi/contracts';

// Or import from @luxfi/standard directly
import { GovernorAbi, StrategyAbi, SubDAOAbi } from '@luxfi/standard';
```
...
Lux DAO follows a modified style of semantic versioning (https://semver.org/) specific to a smart contract use case.
...
Lux DAO contracts are deployed on:

- **Lux Mainnet** - Primary deployment
- **Lux Testnet** - Testing and staging
- **Zoo Network** - Zoo Labs Foundation
- **Pars Network** - Pars DAO governance
- **Ethereum Mainnet** - Legacy support
- **Base, Polygon, Optimism** - L2 deployments

## Related Packages

- `@luxfi/standard` - Core Lux standard library with governance ABIs
- `@luxfi/sdk` - TypeScript SDK for interacting with Lux contracts
